...
def dohvati_tecaj(valuta):
    try:
        ...
        response.raise_for_status()
        podaci = response.json()
        return float(podaci["rates"].get(valuta, 0))
    except requests.exceptions.RequestException as e:
        messagebox.showerror("Greška", f"Neuspješno dohvaćanje tečaja: {e}")
        return 0
...
vasa_valuta_entry = ttk.Combobox(vasa_valuta_label, values=valute, font=("Helvetica", 16))
...
unos_vlaute_za_konverziju = ttk.Combobox(konverzija, values=valute, font=("Helvetica", 16))
